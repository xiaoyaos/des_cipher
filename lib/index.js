...
class DES {
   ...
   * @param {*} cszpKey             密码
   */
  static EncryptFile(cszpSourceFileName, cszpPwdFileName, cszpKey) {
    ...
    cipher.on('finish', () => { console.log("zhengque"); cipher.close();});
  ...
  static async DecryptFile(cszpPwdFileName, cszpResultFileName, cszpKey) {
    ...
    plain.on("open", ()=>{
      ...
      if (count == 7) {/*有填充*/
        ...
        let cipbuf = Buffer.from(plainBlockArr); 
        plain.write(cipbuf);
      }
    });

    plain.on('error', (err) => { console.log("cuowu"); });
    plain.on('finish', () => { console.log("zhengque"); cipher.close();});
    return OK;
  }
